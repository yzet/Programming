...
//                printf("%s\n", "Agency added");
                ...
                case 7:
                    ...
                    break;
                case 8:
                    print_agencies_with_two_or_more_words_in_field(&agencies_list);
                    break;
                default:
...
void print_main_menu(char *command) {
    ...
    printf("%s\n", "8 - Print agencies with two pr more words in field");
...
//    agency->director.email = email;
    ...
    if (*(input) == 'y')
        is_weekends = true;
//    agency->is_weekends = is_weekends;
    // free(input);
...
//    agency->years_on_market = (size_t)strtol(input, NULL, 10);
    ...
    if (is_std_regex(surname) && is_std_regex(name) && is_std_regex(email) && is_std_regex(agency_name)) {
        agency->director.surname = surname;
        agency->director.name = name;
        agency->director.email = email;
        agency->is_weekends = is_weekends;
        ...
        free(input);
        printf("%s", "Error in input, repeat\n");
    }
}
