...
int add_element_to_list_by_index(struct AgenciesList *agencies_list, struct Agency *element, size_t index) {
#ifdef DEBUG
    print_function_and_now_time(__FUNCTION__, time(NULL));
#endif
    ...
    if (index == 0) {
        struct AgenciesListElement *new_object = (struct AgenciesListElement *) malloc(
                sizeof(struct AgenciesListElement));
...
int get_element_with_city_from_index(struct AgenciesList *agencies_list, size_t from_index, enum Cities city,
                                     struct AgenciesListElement **element) {
#ifdef DEBUG
    print_function_and_now_time(__FUNCTION__, time(NULL));
#endif
    size_t list_length = get_list_length(agencies_list);
...
int set_agencies_list_from_text_array(struct AgenciesList *agencies_list, char **agencies_text_array,
    ...
    for (size_t agency_index = 0; agency_index < count_of_agencies; agency_index++) {
        ...
        if (strtol(*(agency_info + IsWeekendsField), NULL, 10) == 1)
            is_weekends = true;
        struct Agency new_agency = {is_weekends, *(agency_info + AgencyNameField),
                                    (size_t) strtol(*(agency_info + YearsOnMarketField), NULL, 10),
                                    {*(agency_info + SurnameField), *(agency_info + NameField),
                                     *(agency_info + EmailField)},
                                    (unsigned int) strtol(*(agency_info + CityField), NULL, 10)};
...
void print_agencies_with_two_or_more_words_in_field(struct AgenciesList *agenciesList) {
    for (size_t i = 0; i < get_list_length(agenciesList); i++) {
        struct AgenciesListElement *agenciesListElement;
        get_element_by_index(agenciesList, &agenciesListElement, i);
        if (is_two_or_more_words_regex(agenciesListElement->data.name) ||
            (is_two_or_more_words_regex(agenciesListElement->data.director.name)
             || is_two_or_more_words_regex(agenciesListElement->data.director.surname) ||
             is_two_or_more_words_regex(agenciesListElement->data.director.email))) {
            print_agency(&(agenciesListElement->data), stdout);
        }
    }
}