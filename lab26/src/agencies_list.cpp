...
void AgenciesList::SortByComparator(int(*comparator)(const Agency &left, const Agency &right)) {
    for (size_t i = 0; i < length; i++) {
        for (size_t j = 0; j < length - 1; j++) {
            if (comparator(GetElementByIndex(j).GetData(), GetElementByIndex(j + 1).GetData()) > 0) {

                SwapListElements(&GetElementByIndex(j),
                                 &GetElementByIndex(j + 1));
            }
        }
    }
}
...
AgenciesListElement &AgenciesList::operator[](int index) const {
...
ofstream &operator<<(ofstream &output, const AgenciesList &agencies_list) {
    for (size_t i = 0; i < agencies_list.length; i++) {
        output << agencies_list[(int) i].GetData();
    }
    return output;
...
istream &operator>>(istream &input, AgenciesList &agencies_list) {
